...
allschools_lat <- inner_join(data2013temp, latlong, by=c("UNITID" = "UnitID"))
...
tomap <- allschools_lat %>% 
  select(NPT41, NPT42, NPT43, NPT44, NPT45, Ziplat, INSTNM, Longitude, Latitude, STABBR, UG)
...
inputPanel(
  sliderInput("income", label = "Family Income:",
              min = 0, max = 200000, value = 50000, step = 500) ,
  
  sliderInput("max", label = "Maximum Amount:",
...
renderPlot({
  ...
  var <- ifelse(input$income <= 30000, 1, 
  ...
  tomapshiny <- tomap %>% filter(tomap[var] <= input$max)
